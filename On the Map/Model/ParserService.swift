...
class ParserService : NSObject {
    ...
    func taskForGETMethod<D: Decodable>(_ method: String,decode:D.Type, completionHandlerForGET: @escaping (_ result: AnyObject?, _ error: NSError?) -> Void) -> URLSessionDataTask {
        ...
        let request = NSMutableURLRequest(url: ParserService.tmdbURLFromWithoutParameters( withPathExtension: method))
    ...
    class func tmdbURLFromWithoutParameters( withPathExtension: String? = nil) -> URL {
        ...
        components.scheme = ParserService.Constants.ApiScheme
        components.host = ParserService.Constants.ApiHost
        components.path = ParserService.Constants.ApiPath + (withPathExtension ?? "")
    ...
    class func sharedInstance() -> ParserService {
        struct Singleton {
            static var sharedInstance = ParserService()
        }
